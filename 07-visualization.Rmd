...
```{r, fig.cap = "Ankle torque by group and time", message=FALSE, warning=FALSE}
...
```{r ggthemes, fig.cap= "Default themes in ggplot.", fig.margin = TRUE, echo = FALSE, out.width = "50%", fig.align='center'}
knitr::include_graphics(c("images/visualization-themes.png"))
```
...
```{r strnshape, fig.cap="strength: shape and color"}
...
```{r shapelegend, fig.cap="Legend removed"}
ggplot(strn, aes(ktorq, atorq, color =group)) +
  geom_point() +
...
```{r shapelegend-moved, fig.cap='Legend on the bottom'}
ggplot(strn, aes(ktorq, atorq, color =group)) +
  geom_point() + 
...
```{r shapelegend-moved2, fig.cap='Legend at a point'}
ggplot(strn, aes(ktorq, atorq, color =group)) +
  geom_point() + 
...
```{r ktorqbar, fig.cap='Knee torque by group', message=FALSE, warning=FALSE}
...
```{r ktorqbarerrors, fig.cap='Knee torque by group with error bars as 1SD', message=FALSE, warning=FALSE}
...
```{r customcol, fig.cap='Knee torque by group with error bars as 1SD', message=FALSE, warning=FALSE}
...
```{r kneehist, fig.cap = "Histogram of knee torque", message=FALSE, warning=FALSE}
ggplot(data = strn) +
...
```{r boxplot, fig.cap='Boxplot of knee torque by group and time'}
ggplot(data = strn) +
